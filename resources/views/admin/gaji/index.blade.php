                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse($employees as $employee)
                            @php
                                ...
                                $currentMonthSlip = $employee->gajis->first();
                            @endphp
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-4 py-3 table-cell">
                                    <span class="px-2 py-1 text-xs font-semibold rounded-full 
                                        @if(($currentMonthSlip ? $currentMonthSlip->status : 'pending') === 'pending')
                                            bg-yellow-100 text-yellow-800
                                        @elseif(($currentMonthSlip ? $currentMonthSlip->status : 'pending') === 'approved')
                                            bg-green-100 text-green-800
                                        @else
                                            bg-gray-100 text-gray-800
                                        @endif">
                                        {{ $currentMonthSlip ? $currentMonthSlip->status : 'pending' }}
                                    </span>
                                </td>
                                <td class="px-4 py-3 table-cell">
                                    <div class="flex space-x-2">
                                        @if(!$currentMonthSlip || $currentMonthSlip->status === 'pending')
                                        <button onclick="openSlipGajiModal({{ $employee->employee_id }}, '{{ addslashes($employee->name) }}', {{ $employee->gaji_pokok }}, {{ $data['workdays'] }}, {{ $data['absent_days'] }}, {{ $data['hadir'] }}, {{ $data['telat'] }}, {{ $data['absen'] }})"
...
<div id="slipGajiModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full hidden z-50">
    <div class="relative mx-auto mt-32 mb-10 p-0 w-full max-w-2xl shadow-2xl rounded-2xl bg-white">
        ...
        <form id="slipGajiForm" method="POST" class="space-y-4 px-8 pb-8 pt-2">
            ...
            <div class="mb-4">
                <label class="block text-gray-700 text-base font-semibold mb-1">Kehadiran</label>
                <div class="grid grid-cols-3 gap-4">
                    <div>
                        <span class="text-sm text-gray-500">Hadir</span>
                        <div id="slip_hadir_count" class="text-lg font-medium text-green-600">0</div>
                    </div>
                    <div>
                        <span class="text-sm text-gray-500">Terlambat</span>
                        <div id="slip_telat_count" class="text-lg font-medium text-yellow-600">0</div>
                    </div>
                    <div>
                        <span class="text-sm text-gray-500">Absen</span>
                        <div id="slip_absen_count" class="text-lg font-medium text-red-600">0</div>
                    </div>
                </div>
            </div>
            
            <div class="mb-4">
                <label for="komponen_tambahan" class="block text-gray-700 text-base font-semibold mb-1">Komponen Tambahan</label>
                <div class="flex items-center">
                    <span class="inline-block mr-2 text-gray-500 font-semibold">Rp</span>
                    <input type="number" name="komponen_tambahan" id="komponen_tambahan" value="0" min="0"
                        class="w-full rounded-xl border-gray-300 shadow-sm bg-gray-50 focus:border-indigo-500 focus:ring-indigo-500 p-2 text-base">
                </div>
            </div>
            
            <div class="mb-4">
                <label for="slip_potongan" class="block text-gray-700 text-base font-semibold mb-1">Potongan</label>
                <div class="flex items-center">
                    <span class="inline-block mr-2 text-gray-500 font-semibold">Rp</span>
                    <input type="number" name="potongan" id="slip_potongan" value="0" min="0" readonly
                        class="w-full rounded-xl border-gray-300 shadow-sm bg-gray-100 focus:border-indigo-500 focus:ring-indigo-500 p-2 text-base">
                </div>
            </div>
...
function openSlipGajiModal(employeeId, employeeName, gajiPokok, workdays, absentDays, hadir, telat, absen) {
    const modal = document.getElementById('slipGajiModal');
    ...
    // Calculate potongan based on absent days
    let potongan = 0;
    if (workdays > 0) {
