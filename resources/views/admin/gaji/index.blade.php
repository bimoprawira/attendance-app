                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    <th class="px-4 py-3 text-left table-header font-medium text-gray-500 uppercase">Info
                                        Karyawan</th>
                                    <th class="px-4 py-3 text-left table-header font-medium text-gray-500 uppercase">Gaji
                                        Pokok</th>
                                    <th class="px-4 py-3 text-left table-header font-medium text-gray-500 uppercase">Status
                                        Slip Gaji</th>
                                    <th class="px-4 py-3 text-left table-header font-medium text-gray-500 uppercase">Aksi
                                    </th>
                                </tr>
                            </thead>
                                ...
                                @foreach ($employees as $employee)
                                    @php
                                        ...
                                        $currentMonthSlip = $employee->gajis->first(
                                            fn($g) => Carbon::parse($g->periode_bayar)->format('Y-m') === $month,
                                        );
                                    @endphp
                                    <tr class="hover:bg-gray-50">
                                        <td class="px-4 py-3 table-cell">
                                            <div class="flex items-center">
                                                <div
                                                    ...
                                                    <span
                                                        class="text-indigo-600 font-medium text-lg">{{ strtoupper(substr($employee->name, 0, 1)) }}</span>
                                                </div>
                                                <div class="ml-4">
                                                    <div class="text-base font-medium text-gray-900">{{ $employee->name }}
                                                    </div>
                                        ...
                                        <td class="px-4 py-3 table-cell">
                                            <div class="flex space-x-2">
                                                @if (!$currentMonthSlip)
                                                    <button
                                                        onclick="openSlipGajiModal({{ $employee->employee_id }}, '{{ $employee->name }}', {{ $employee->gaji_pokok }}, {{ $data['workdays'] }}, {{ $data['absent_days'] }}, {{ $data['hadir'] }}, {{ $data['telat'] }}, {{ $data['absen'] }})"
